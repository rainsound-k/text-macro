...
| 파일 | 대상 |
|------|------|
| `TextMacro_*_aarch64.dmg` | Apple Silicon (M1/M2/M3) |
| `TextMacro_*_x64.dmg` | Intel Mac |

DMG를 열고 **TextMacro**를 `Applications` 폴더로 드래그하세요.
서명되지 않은 빌드이므로 처음 실행 시 경고가 뜨면 앱을 **우클릭 → 열기**로 실행합니다.
